...
export default function Table(props: OwnProps) {
  ...
  const stations = useSelector((state: any) => state.citybikesStations);
  const { likedStations } = stations;
  ...
  const isLikeStation = (item: INetworkItem) => {
    return likedStations.some(
      (element: INetworkItem) => element.id === item.id
    );
  };
    ...
    <div className="table">
      {data.map((item: INetworkItem) => (
        ...
        >
          {item.title}
          {item.like ? (
            <div onClick={() => toggleLikedStation(item)}>
              <svg
                className="table__like"
              ...
              >
                <path
                  ...
                  fill={isLikeStation(item) ? "#ff6243" : "#fff"}
                />
                <path
